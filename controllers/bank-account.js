...
const getBankAcc = async (req, res, next) => {
  try {
    ...
    if (bkAccs[0]._id.toString() !== user.id && user.role !== 'admin') {
...
const updateBankAcc = async (req, res, next) => {
  try {
    ...
    const userBkAcc = req.user.id;
    if (userBkAcc !== bankAcc.user_id && req.role !== 'admin') {
...
const createBankAcc = async (req, res, next) => {
  try {
    ...
    let db = mongodb.getDb();
    const bankAcc = req.body;
    bankAcc.ballance = 500;

    const user = await usersController.getUser(bankAcc.user_id);
    if (!user) {
